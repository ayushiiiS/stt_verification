#!/usr/bin/env python3
"""Generate Sarvam STT transcripts for IndiaMART calls."""
...
CSV_PATH = BASE_DIR / "indiamart_final63_public_urls.csv"
TRANSCRIPTS_PATH = BASE_DIR / "indiamart_63_transcripts.json"
SARVAM_PATH = BASE_DIR / "indiamart_sarvam_transcripts.json"
SARVAM_LOCK_PATH = BASE_DIR / "indiamart_sarvam_transcripts.lock"
PROGRESS_PATH = BASE_DIR / "indiamart_stt_progress.json"
...
def load_calls() -> list[dict]:
    ...
    with CSV_PATH.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            call_id = call_id_from_url(row.get("public_url") or row.get("url") or "")
            if not call_id:
                continue
            audio_by_id[call_id] = row.get("public_url") or row.get("url") or ""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate IndiaMART Sarvam STT transcripts")
    ...
    print(
        f"IndiaMART total: {total} | already done: {skipped} | to process: {pending_total} | workers: {args.workers}",
        flush=True,
    )
